...
mydouble *multinormal_sample(int m, int n, mydouble a[], mydouble mu[], int *seed);
double r8_uniform_01(int *seed);
void r8mat_print(int m, int n, double a[], string title);
void r8mat_print_some(int m, int n, double a[], int ilo, int jlo, int ihi,
	int jhi, string title);
void r8mat_write(string output_filename, int m, int n, double table[]);
mydouble *r8po_fa(int n, mydouble a[]);
mydouble *r8vec_normal_01_new(int n, int *seed);
void r8vec_print(int n, double a[], string title);
mydouble *r8vec_uniform_01_new(int n, int *seed);
void timestamp();
...
//
{
	...
	for (i = 0; i < n; i++)
	{
		...
		*seed = 16807 * (*seed - k * 127773) - k * 2836;
		...
		r[i] = (mydouble)(*seed) * 4.656612875E-10;
	}
